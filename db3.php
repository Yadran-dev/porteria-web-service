...
class SqlSrv3
{
    ...
    public function querySecure($query, $params)
    {
    ...
    public function fetchArraySecure($query, $params)
    {
        $type = SQLSRV_FETCH_ASSOC;
        $stmt = $this->querySecure($query, $params);
        $a_array = array();
    ...
    public function get($tableName, $feilds = '*', $type = SQLSRV_FETCH_ASSOC, $order = 'DESC')
    {
        ...
        $sql = "SELECT $feildsString FROM $tableName";
    ...
    public function get_by_id($tableName, $id = null)
    {
      ...
      public function buscarporid($tableName,$campo, $id = null)
    {
        $sql = "SELECT * FROM $tableName WHERE $campo = $id";
        $preparedStatement = sqlsrv_prepare($this->connection, $sql);
        $result = sqlsrv_execute($preparedStatement);
        if ($result === true) {
            while ($row = sqlsrv_fetch_array($preparedStatement, SQLSRV_FETCH_ASSOC)) {
